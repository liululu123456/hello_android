...
class ComposeTweetsActivity  : AppCompatActivity() {
    ...
    private val tweetListType = object : TypeToken<List<Tweet>>() {}.type
    private val dataset:List<Tweet> = gson.fromJson(json, tweetListType)
    val toDisplayData: List<Tweet> = dataset.filter { it.content != null }.toList()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContent {
            TweetList(toDisplayData)
        }
    }
    ...
    fun TweetItem(tweet: Tweet) {
        val avatar: Painter = rememberImagePainter(data = tweet.sender?.avatar)
        ...
        Row(modifier = Modifier.padding(16.dp)) {
            ...
            Column(modifier = Modifier.padding(start = 16.dp)) {
                Text(tweet.sender!!.nick, fontSize = 16.sp)
                tweet.content?.let { Text(it, fontSize = 14.sp) }
            }
        }
    }
}